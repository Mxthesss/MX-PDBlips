...
Citizen.CreateThread(function()
    ...
    if Config.useMXMulti then
        while exports['mx_multichar']:getChar() == nil do
            Wait(100)
        end
    else
    ...
    if Config.useMXMulti then
	    PlayerData.char = exports['mx_multichar']:getChar()
    end
...
if Config.useBaseEvents then
    AddEventHandler("baseevents:enteredVehicle", function(veh, seat, vehiclelabel)
        ...
        if onDuty then
            ...
            TriggerServerEvent('mx_pdblips:enteredVeh', cfg)
        end
    end)
    ...
    AddEventHandler("baseevents:leftVehicle", function(veh, seat, vehiclelabel)
        ...
        if lastSirenState then
            ...
            TriggerServerEvent('mx_pdblips:toggleSiren', false)
        end
        if onDuty then
            TriggerServerEvent('mx_pdblips:leftVeh')
        end
    end)
else
    Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while inVeh do
            if IsVehicleSirenOn(veh) and not lastSirenState then
                lastSirenState = true
                TriggerServerEvent('mx_pdblips:toggleSiren', true)
...
function goOnDuty()
    onDuty = true
    TriggerServerEvent('mx_pdblips:setDuty', true)
    ...
    if inVeh then
        TriggerServerEvent('mx_pdblips:enteredVeh', Config.emergencyJobs[PlayerData.job.name].vehBlip[GetEntityModel(GetVehiclePedIsIn(PlayerPedId(), false))])
    end
end
AddEventHandler('mx_pdblips:goOnDuty', goOnDuty)

function goOffDuty()
    onDuty = false
    TriggerServerEvent('mx_pdblips:setDuty', false)
...
AddEventHandler('mx_pdblips:goOffDuty', goOffDuty)

AddEventHandler('mx_pdblips:toggleDuty', function(bool)
    if bool then
        goOnDuty()
    else
        goOffDuty()
...
RegisterNetEvent('mx_pdblips:removeUser')
AddEventHandler('mx_pdblips:removeUser', function(plyId)
...
RegisterNetEvent('mx_pdblips:receiveData')
AddEventHandler('mx_pdblips:receiveData', function(myId, data) -- ugly ass event
