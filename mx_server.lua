...
Citizen.CreateThread(function()
    while true do
        ...
        for player, kekw in pairs(dutyTable) do
            TriggerClientEvent('mx_pdblips:receiveData', player, player, sendTable)
        end
        Citizen.Wait(1000)
    end
end)

RegisterNetEvent('mx_pdblips:setDuty')
AddEventHandler('mx_pdblips:setDuty', function(onDuty)
    local src = source
    ...
    if onDuty then
        ...
        if dutyTable[src] then
            ...
            dutyTable[src] = nil
            for k, v in pairs(dutyTable) do
                TriggerClientEvent('mx_pdblips:removeUser', k, src)
            end
        else
...
RegisterNetEvent('mx_pdblips:enteredVeh')
AddEventHandler('mx_pdblips:enteredVeh', function(vehCfg)
    local src = source
...
RegisterNetEvent('mx_pdblips:leftVeh')
AddEventHandler('mx_pdblips:leftVeh', function()
    local src = source
...
RegisterNetEvent('mx_pdblips:toggleSiren')
AddEventHandler('mx_pdblips:toggleSiren', function(isOn)
    local src = source
    local playerJob = ESX.GetPlayerFromId(src).getJob()
    if isOn then
